...
@Component({
  ...
  template: `
    ...
    <section class="bloque">
      <div class="calendario__barra">
        ...
        <div class="calendario__mando">
          <button type="button" class="admin-mini" (click)="moverMes(-1)" aria-label="Mes anterior">‹</button>
          <span class="calendario__mes">{{ tituloMes() }}</span>
          <button type="button" class="admin-mini" (click)="moverMes(1)" aria-label="Mes siguiente">›</button>
    ...
    <!-- Exportar -------------------------------------------------------- -->
    <section class="bloque">
      <h3 class="bloque__titulo etiqueta">Imprimir o guardar en PDF</h3>
      ...
      <div class="exportar">
        <button type="button" class="boton" (click)="imprimir('dia')">Del día</button>
        <button type="button" class="boton" (click)="imprimir('semana')">De la semana</button>
        <button type="button" class="boton" (click)="imprimir('mes')">Del mes</button>
      </div>
    </section>

    <!-- Hoja que sale por la impresora. Oculta en pantalla; la saca la regla
         @media print de styles.scss. -->
    @if (impresion(); as hoja) {
      <div class="impresion">
        <header class="impresion__cabecera">
          <h1>{{ restaurante().nombre }}</h1>
          <p class="impresion__rango">{{ hoja.titulo }}</p>
          <p class="impresion__totales">
            {{ hoja.reservas.length }} {{ hoja.reservas.length === 1 ? 'reserva' : 'reservas' }} ·
            {{ hoja.comensales }} comensales
          </p>
        </header>

        @for (grupo of hoja.dias; track grupo.dia) {
          <section class="impresion__dia">
            <h2>{{ diaLargo(grupo.dia) }}</h2>
            <table>
              <thead>
                <tr>
                  <th>Hora</th>
              ...
              <tbody>
                @for (r of grupo.reservas; track r.id) {
                  <tr>
                    <td>{{ r.hora }}</td>
                    <td>{{ r.nombre }}</td>
                    <td>{{ r.personas }}</td>
          ...
          <p class="impresion__vacio">No hay reservas confirmadas en este periodo.</p>
        }

        <footer class="impresion__pie">
          Listado generado el {{ fechaCortaDe(hoy) }} · {{ restaurante().telefono }}
        </footer>
      </div>
    }
  `,
  styles: `
    ...
    .peticion {
      ...
      border: 1px solid rgba(226, 112, 30, .45);
    }

    .peticion__cuerpo { flex: 1 1 260px; min-width: 0; }

    .peticion__nombre { font-size: 15px; font-weight: 700; }
    ...
    .peticion__acciones { display: flex; flex-wrap: wrap; gap: 8px; }
    .peticion__acciones .boton { padding: 12px 20px; }
    ...
    .calendario__mando { display: flex; align-items: center; gap: 6px; }
    ...
    .dia {
      ...
      transition: background .25s var(--suave), color .25s var(--suave);

      &:hover { background: rgba(46, 46, 46, .12); }
    }

    .dia__numero { font-size: 13px; font-weight: 700; }

    .dia--fuera { opacity: .3; }

    .dia--hoy { box-shadow: inset 0 0 0 2px rgba(46, 46, 46, .35); }
    ...
    .dia--ocupado { background: var(--naranja); color: var(--blanco); }
    .dia--ocupado:hover { background: #c85f14; }
    ...
    .dia--elegido:hover { background: var(--tinta); }
    ...
    .exportar { display: flex; flex-wrap: wrap; gap: 10px; }
    ...
    @media (max-width: 620px) {
      .peticion__acciones { width: 100%; }
      .peticion__acciones .boton,
      .peticion__acciones .admin-mini { flex: 1; }
      .calendario { padding: 10px; }
      .dia { border-radius: 11px; }
      .dia__numero { font-size: 12px; }
    }
  `,
})
export class AdminReservas {
  ...
  /** Hoja preparada para la impresora; nula mientras no se pide. */
  readonly impresion = signal<{
    titulo: string;
    reservas: Reserva[];
    comensales: number;
    dias: { dia: string; reservas: Reserva[] }[];
  } | null>(null);
  ...
  imprimir(alcance: Alcance): void {
    ...
    // Agrupadas por dia: en la mesa del restaurante se leen jornada a jornada.
    ...
    this.impresion.set({
      titulo,
      reservas: lista,
      comensales: lista.reduce((s, r) => s + r.personas, 0),
      dias: [...porDia.entries()].map(([dia, reservas]) => ({ dia, reservas })),
    });

    // El dialogo del navegador se abre cuando la hoja ya esta pintada.
    setTimeout(() => {
      window.print();
      this.impresion.set(null);
    }, 60);
  }

  private rango(alcance: Alcance, referencia: string): {
    desde: string;
    hasta: string;
    titulo: string;
  } {
